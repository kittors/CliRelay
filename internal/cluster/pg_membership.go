...
func (p *pgNode) register(ctx context.Context) error {
	...
	if stillActive {
		log.Warnf("cluster: node id %q heartbeated within the last 20s before this process started; "+
			"that is expected right after a crash, otherwise another process uses the same node id and "+
			"the two will drop each other's events (set a unique cluster.node-id per process)", p.nodeID)
	}
	return nil
}
...
// reclaim rewrites this node's row after it was replaced or deleted. A
// replacement means a second process registered the same node ID; both keep
// logging this until one of them is renamed.
func (p *pgNode) reclaim(ctx context.Context, leader bool) {
	if time.Since(p.lastConflictWarn) >= warnEvery {
		...
		log.Errorf("cluster: the membership row of node %q was replaced or removed by someone else; "+

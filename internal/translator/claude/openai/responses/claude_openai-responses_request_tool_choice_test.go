...
const toolChoiceTestTools = `[
	{"type":"function","name":"tool_a","description":"a","parameters":{"type":"object","properties":{}}},
	{"type":"function","name":"tool_b","description":"b","parameters":{"type":"object","properties":{}}}
]`

const toolChoiceTestInput = `[{"type":"message","role":"user","content":[{"type":"input_text","text":"hi"}]}]`

func TestConvertOpenAIResponsesRequestToClaude_ToolChoiceMapping(t *testing.T) {
	cases := []struct {
		name       string
		toolChoice string
		...
		// The Responses API names the function at the top level; only the Chat
		// Completions shape was read, so this one reached Claude with an empty name.
		{name: "named responses shape", toolChoice: `{"type":"function","name":"tool_b"}`, want: `{"type":"tool","name":"tool_b"}`},
		{name: "named chat shape", toolChoice: `{"type":"function","function":{"name":"tool_b"}}`, want: `{"type":"tool","name":"tool_b"}`},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			input := `{"model":"gpt-5","input":` + toolChoiceTestInput +
				`,"tools":` + toolChoiceTestTools + `,"tool_choice":` + tc.toolChoice + `}`
			result := ConvertOpenAIResponsesRequestToClaude("test-model", []byte(input), false)
...
func TestConvertOpenAIResponsesRequestToClaude_ToolChoiceNoneWithoutToolsOmitted(t *testing.T) {
	cases := []struct {
		name  string
		field string
	}{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			input := `{"model":"gpt-5","input":` + toolChoiceTestInput + tc.field + `,"tool_choice":"none"}`
			result := ConvertOpenAIResponsesRequestToClaude("test-model", []byte(input), false)

...
// prober sends the readiness probe to each node.
type prober struct {
	client  *http.Client
	host    string
	path    string
...
func (p *prober) probe(ctx context.Context, ip string) probeResult {
	...
	start := time.Now()
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://"+ip+p.path, nil)
	if err != nil {
...
// probeAll probes every node concurrently and returns results in node order.
func (p *prober) probeAll(ctx context.Context, nodes []Node) []probeResult {
	results := make([]probeResult, len(nodes))
	var wg sync.WaitGroup
	for i, node := range nodes {
		wg.Go(func() { results[i] = p.probe(ctx, node.IP) })
	}
	wg.Wait()
	return results
}
